...
- Some images require a one-time non-interactive setup, migration, seed, repair, or administrative command before their normal process can start. For any managed Service, runServiceInitializationCommand starts a disposable container with the Service's exact image, environment, user, working directory, persistent storage, and private App network, but without published ports or the Service's private DNS alias. It does not start, stop, or alter the original Service and returns no command output.
- Use runServiceInitializationCommand only when documentation or a concrete failure establishes that a specific command is required. Inspect the Service first, pass an argument array directly without a shell, and never use it for exploration. Never put credentials in command arguments; configure protected environment variables first. Explain that the command can change persistent data and contact private dependencies, then call the approval-requiring tool in the same response. If the Service is running, account for concurrent access to shared storage and stop it first when the operation requires exclusive access. Do not repeat non-idempotent commands merely because their output is withheld.
...
export async function createChatResponse(
  ...
  const tools = {
    ...
    runServiceInitializationCommand: tool({
      description: 'Run an approved one-time initialization, migration, seed, repair, or administrative command for any managed Service, including a stopped or image-based Service. A disposable container receives the same image, environment, storage, and App network without publishing ports. Arguments execute directly, the original Service is unchanged, and output is withheld. Never pass credentials as arguments.',
      inputSchema: z.object({ appId, serviceId, command: z.array(z.string().min(1).max(4096)).min(1).max(32) }),
      execute: ({ appId, serviceId, command }) => withProgress(() => docker.runServiceInitializationCommand(appId, serviceId, command)),
    }),
