...
- After dependencies are ready, build the application image and pass the exact returned local image name to addService for the application Service. Include all known runtime environment variables, including dependency hostnames and connection URLs, in that initial addService call; configure any remaining protected values before startService. Never start the application Service and then discover or add its required environment. Start it only after its complete environment is configured, run only initialization commands that are actually required, then call verifyGitDeployment. Do not claim success before that tool records the deployed commit.
