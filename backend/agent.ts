...
- To deploy latest Git changes to an EXISTING App, inspect its state, call refreshGitRepository for its configured branch, then inspectRepository and buildRepositoryImage. Pass the exact returned image to deployRepositoryImage for each existing application Service using this repository; do not replace database/cache Services. Preserve the App, Service identities, domains, protected environment, and storage; never delete or create a replacement App merely to update code. Refresh preserves local deployment files and refuses conflicting edits rather than discarding them. If changed is false and currentCommit equals resolvedCommit with a verified running deployment, report that it is already current instead of rebuilding. Otherwise continue the unfinished deployment. Inspect new configuration/migration requirements before replacement, retaining existing secrets; stop the affected Service first if approved pre-start initialization is needed. Image replacement preserves running/stopped state and briefly interrupts running Services, so do not promise zero downtime. Start stopped application Services when ready and call verifyGitDeployment before claiming the latest commit is deployed. Restart/recreate alone does not fetch or build Git changes.
...
- Start with the compact inspection returned by createGitApp or inspectRepository after a refresh. Read additional repository files only when needed. Never seek secrets, .env contents, keys, credentials, unrelated personal data, or the contents of other Apps.
...
export async function createChatResponse(
  ...
  const tools = {
    ...
    refreshGitRepository: tool({
      description: 'Fetch the latest commit of an existing Git App\'s configured branch using its existing credentials, then safely update its checkout. Preserves local deployment files; refuses conflicting edits. Does not change running Services. Returns changed and resolved/current commits. Inspect, build, deployRepositoryImage, then verify to finish the update.',
      inputSchema: z.object({ appId }),
      execute: ({ appId }) => withProgress(() => docker.refreshGitRepository(appId, reportProgress)),
    }),
    deployRepositoryImage: tool({
      description: 'Replace one existing application Service with this App\'s latest successfully built repository image. Pass the exact image from buildRepositoryImage. Preserves App/Service identity, environment including protected values, storage, domains, ports and running/stopped state. Briefly interrupts a running Service. Does not update dependencies or mark the commit verified.',
      inputSchema: z.object({ appId, serviceId, image: z.string().min(1).max(255) }),
      execute: ({ appId, serviceId, image }) => withProgress(() => docker.deployRepositoryImage(appId, serviceId, image)),
    }),
