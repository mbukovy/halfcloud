...
- If you ultimately cannot complete a request because the required capability or tool is unavailable, unsupported, or did not work after reasonable attempts, explain the limitation and say: "Please submit an issue at https://github.com/mbukovy/halfcloud/issues and my creators will look at it." Do not use this message for a recoverable error or when you only need information, approval, or an external action from the user. Do not tell the user to include an App name or App ID in an issue; those local identifiers do not help diagnose the problem. If useful, ask them to include the exact failure message instead.
...
- During initial deployment or explicit recipe repair, start with the compact inspection returned by createGitApp or inspectRepository. Read additional repository files only when needed. Never seek secrets, .env contents, keys, credentials, unrelated personal data, or the contents of other Apps.
...
- Use createGitApp only once for a deployment. A failed build may be diagnosed from its bounded logs and repository reads, adjusted with a deployment-file write, and retried up to the enforced limit. If all three attempts fail, apply a concrete correction first, then call retryRepositoryBuild to open one new bounded build cycle without recreating the App. Never reset attempts merely to repeat an unchanged failing build.
...
- During initial deployment, prepare a complete self-contained runtime image for each application Service, including every runtime import and asset. Build it and pass the exact returned image to addService, or use deployRepositoryImage only for explicitly requested preparation/repair of an existing Service. Include all known runtime environment variables in the initial addService call and configure protected values before startService. Run only required approved initialization, then start and call verifyGitDeployment. Verification saves a separate updating Dockerfile, ignore rules, build context, immutable image identity, and health path for every repository-built Service outside the checkout. These are the recipes future non-AI updates will reuse. Use a real readiness endpoint where available. Never claim success before verification. The deterministic updater never replays initialization or migrations; updates requiring data/configuration changes need explicit preparation.
...
export async function createChatResponse(
  ...
  const tools = {
    ...
    refreshGitRepository: tool({
      ...
      inputSchema: z.object({ appId }),
      execute: ({ appId }) => withProgress(() => docker.refreshGitRepository(appId, reportProgress)),
    }),
    deployRepositoryImage: tool({
      description: 'For explicitly requested deployment preparation or recipe repair only, replace a Service with a successfully built repository image and check readiness before removing the previous container. Preserve configuration and storage. Then verifyGitDeployment to save its recipe. For ordinary updates always use updateGitApp instead.',
      inputSchema: z.object({ appId, serviceId, image: z.string().min(1).max(255), healthPath: z.string().startsWith('/').max(500).optional().describe('Readiness endpoint for explicit recipe preparation; otherwise reuse the saved endpoint') }),
      execute: ({ appId, serviceId, image, healthPath }) => withProgress(() => docker.deployRepositoryImage(appId, serviceId, image, healthPath)),
    }),
    ...
    retryRepositoryBuild: tool({
      description: 'Open another bounded three-attempt build cycle for a Git-backed App after all three previous builds failed and a concrete correction has been applied. Preserves the App, repository checkout, Services, and data.',
      inputSchema: z.object({ appId }),
      execute: ({ appId }) => docker.retryRepositoryBuild(appId),
    }),
    buildRepositoryImage: tool({
      ...
      inputSchema: z.object({
        ...
        generatedDockerfileContent: z.string().max(131072).optional().describe('Omit when reusing an existing Dockerfile; never send an empty replacement'),
    ...
    verifyGitDeployment: tool({
      description: 'Verify all repository-built Services use their recorded images and remain healthy, then save dedicated per-Service updating Dockerfiles and health paths and record the deployed commit. Supports different images per Service. Required to finish initial deployment or explicit recipe preparation; ordinary updates use updateGitApp.',
      inputSchema: z.object({ appId, serviceId: serviceId.optional(), healthPath: z.string().startsWith('/').max(500).default('/'), serviceHealthPaths: z.record(z.string(), z.string().startsWith('/').max(500)).optional().describe('Per-Service readiness endpoints keyed by Service ID or name, for Apps with multiple web/API Services') }),
      execute: ({ appId, serviceId, healthPath, serviceHealthPaths }) => withProgress(() => docker.verifyGitDeployment(appId, serviceId, healthPath, serviceHealthPaths)),
    }),
